...
describe(name, function () {
  ...
  it('inserts ad dummies in place of ads', function () {
    var result = filter([paragraph1, paragraph4, paragraph1, paragraph4, paragraph1, desktopOutStreamAd, paragraph4, paragraph1, paragraph4, paragraph1]),
      adDummy = {
        viewportMin: desktopOutStreamAd.viewportMin,
        viewportMax: undefined,
        _ref: 'localhost/components/ad-dummy/instances/general'
      };

    expect(_.includes(result, desktopOutStreamAd)).to.equal(false);
    expect(result[5].viewportMin).to.equal(desktopOutStreamAd.viewportMin);
    expect(result[5].viewportMax).to.equal(desktopOutStreamAd.viewportMax);
    expect(result[5]._ref).to.equal(adDummy._ref);
  });

  it('does not insert ad dummies when there are no ads', function () {
    var result = filter([paragraph1, paragraph4, paragraph1, paragraph4, paragraph1]),
      adDummy = {
        viewportMin: desktopOutStreamAd.viewportMin,
        viewportMax: undefined,
        _ref: 'localhost/components/ad-dummy/instances/general'
    ...
    expect(_.includes(result, adDummy)).to.equal(false);
  });
});
